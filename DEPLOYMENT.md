...
`restic`，也没有 `/etc/nav/nav-backup.env`、`/etc/nav/restic-r2.env`、`/usr/local/sbin/nav-backup`、
`/usr/local/sbin/nav-restore-rehearsal` 或 NAV/restic 的 systemd timer。因此当前只能证明每次
受控发布创建备份并完成隔离恢复，不能声称自动异地备份和失败报警已经启用。

2026-08-23 的 `25c9c13` 发布前与发布后备份均在无网络 PostgreSQL 16 临时容器中完成隔离
恢复，均为 17 张表、17 条迁移和逐表行数精确一致；本次没有新迁移，只读迁移校验通过。
本次只重建 `nav-api`；`nav-web`、PostgreSQL、CLIProxyAPI、NPM、Vaultwarden 和 Komari
容器 ID 均未改变。一次性管理员生命周期与清理通过，临时账号、marker 和运行时私有目录
均为 0。证据保存在当前 release 的受限 `evidence` 中。当前仍没有启用计划备份、异地上传、
远端保留清理或主机外失败报警。

2026-08-23 的 `9b05013` 是当前直接应用回滚目标。2026-08-22 的 `5a42279` 是更早的历史恢复
证据：发布前为 16 张表和 16 条迁移，发布后为 17 张表和 17 条迁移；它不再是直接回滚目标。
