...
统一审计和图床删除结果闭环已有待发布候选，但尚未完成生产验收。异地备份、告警、
Passkey、多环境部署和客户自部署文档仍未收口。
...
待发布候选把认证、已认证写操作和 AI 限流迁移到 PostgreSQL。生产切换前必须在每个 API
副本使用的 owner-only `api/.env` 中配置相同的稳定随机值：
...
当前生产 NAV 还没有向用户展示图床返回的来源删除/legacy/缓存撤销细分结果；待发布候选
已完成严格合约校验、结果持久化和保守提示，但仍需真实渠道与双域生产验收。
