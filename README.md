...
2026-09-08追加授权：继续采用 PWA，不购买原生 App 方案。本地新增安全 Google 窗口交接、默认14天/主动信任最长30天会话、主域可选新设备通行密钥（系统 Face ID/Touch ID/设备密码）、可配置 Turnstile，以及 iPhone 风格认证/导航/安全界面。新协议不是恢复旧 Passkey 接口；功能开关默认关闭。详见同一份[方案中的追加范围](./docs/NAV_PRODUCT_SPEC_20260907.md#2026-09-08-追加实施基线pwa与iphone体验)。当前代码未提交、未部署，旧CI不覆盖这批更改。
...
- 系统 SMTP 通知：注册验证、审批结果、运维告警与管理员通道测试；不再开放个人邮箱
...
旧 `/auth/passkeys` 协议固定退役，旧说明仅作历史参考；新的可选 `/auth/device-keys` 必须通过独立开关与验收启用。BM25/本地向量语义搜索、Web Push、块编辑器、多人协作、离线同步、流式
...
19. [Google 登录配置与双域验收](./docs/NAV_GOOGLE_LOGIN_SETUP.md)
