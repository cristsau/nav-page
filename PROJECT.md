...
| 源码完成、待启用 | Passkey/WebAuthn（唯一 RP ID、迁移、API、UI、审计与测试已完成，默认关闭） |
| 未完成 | 自动异地加密备份、外部失联监测、RAG/语义检索、AI 成本面板、编辑器版本历史与块编辑器 |
...
1. 自动异地加密备份、定期隔离恢复演练、失败报警和主机外 dead-man。
2. 按 `docs/NAV_PASSKEYS.md`完成 Passkey 的 CI、迁移和真实设备验收后渐进启用。
3. 提前/离线提醒、定时失效链接检查和命令面板补全。
4. 中文检索与带来源的个人数据助理，并增加 AI 用量/成本可见性。
