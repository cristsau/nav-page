...
- 登录、注册、恢复、已认证写操作和 AI 的 PostgreSQL 共享限流（待生产发布验收）
- 管理员安全审计查询与响应式面板（待生产发布验收）
...
| 待发布候选 | 图床删除结果闭环、PostgreSQL 共享限流、最小化安全审计与管理员面板 |
| 部分完成 | 打开页面时提醒、手动链接检查、安装型 PWA、JSON 数据迁移 |
| 未完成 | 自动异地备份/报警、审计保留/告警、Passkey、RAG/语义检索、AI 成本面板、编辑器版本历史与块编辑器 |
...
1. 通过 CI 并发布验收图片删除结果、共享限流与安全审计候选。
2. 自动备份、异地恢复和失败报警；补审计保留策略后再渐进增加 Passkey。
