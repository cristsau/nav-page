# DOMO NAV — 系统通知与云备份自助配置

## 2026-09-07 当前入口

个人邮箱已从本地候选退出，尚未发布。当前配置入口为“设置 → 登录与系统集成”，分为外部账号登录、
系统 SMTP 通知和加密云备份。系统通知只用于注册验证、审批、运维告警和管理员通道测试；无 IMAP 收件、
个人邮件发送、每日摘要或邮箱 OAuth 设置。密码仍为只写字段，留空保持已有 Secret。

SMTP 流程：保存连接配置 → 测试 SMTP → 启用系统通知/注册验证 → 再保存。主机、账号或密码变更使旧验证失效。
用户管理页保留管理员测试投递。云备份流程不变，Web/API 进程不会执行 root 备份或恢复。
旧邮件配置与密钥暂留作为回滚/历史加密记录恢复材料，不再启动个人邮箱任务。

下方是历史配置记录；其中 SMTP/IMAP 邮箱入口与功能开关不再适用于当前候选。详见
[邮箱下线计划](./NAV_MAIL_RETIREMENT_20260907.md)。
