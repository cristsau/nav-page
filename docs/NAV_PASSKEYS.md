...
> 历史协议文档，不是当前启用手册。R1已将 `/auth/passkeys` 固定退役，下面的旧开关、13.3.x依赖及双RP说明不得用于恢复它。2026-09-08用户另行批准新设备通行密钥，并追加授权双域开放；新协议使用 `/auth/device-keys`、独立表及14.0.x依赖，详见[新协议手册](NAV_DEVICE_KEYS.md)。实际部署与验收状态只看 [CURRENT](../CURRENT.md)。
