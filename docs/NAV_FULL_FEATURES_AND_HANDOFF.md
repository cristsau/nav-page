...
- GitHub PR：`#18` 已合并。
- 当前 `master` 与生产应用发布 SHA：`d0d432b4e0b2e50c25fccb7b45fc609be18c31a0`。
...
- 本轮账户修改、审计删除和 AI 配置来源切换已通过 GitHub CI、生产前备份与隔离恢复演练，并已发布到 OVH。
- 灾难重建后的 OVH 数据边界：截至本次验收为 `1` 个用户、`0` 个导航分组、`0` 个书签、`0` 条笔记；Oracle 旧数据库仍未恢复。不要把“应用已恢复”误写成“Oracle 个人数据已恢复”。
...
6. 只读确认 PR `#18`、`origin/master` 和实时生产仍指向本文记录的发布；如已出现新提交，以实时证据为准。
...
本轮“账户用户名与密码修改 + 安全审计默认折叠和受保护删除 + AI 服务器托管/
自定义配置来源切换”已在 PR #18 合并并发布到 OVH。先只读核对实时 SHA、双域、
容器健康、备份和当前数据库边界；不要重复发布，也不要推断 Oracle 旧个人数据已恢复。
