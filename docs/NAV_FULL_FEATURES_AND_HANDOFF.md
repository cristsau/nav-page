...
- 主分支：`master`。
- 2026-08-21 已核验的 GitHub `origin/master` 与 OVH 生产 SHA 均为
  `369024f9883a87fb0e1bc05a7665cb2e76437ae2`（PR #26 合并点）。
...
- 当前下一批本地候选位于 `codex/nav-maintenance-observability-20260822`：新增迁移 018、后台任务持久状态、管理员状态界面、连续失败阈值/冷却和 Telegram 恢复通知；**尚未推送、合并或发布**。
...
- 本地候选新增后台任务最近成功/失败、耗时、计数与连续失败状态；只持久化白名单计数和错误代码
- 本地候选可在连续失败达到阈值后通知已配置 Telegram 的管理员，并在恢复时通知一次；完整主机离线仍需外部 dead-man
...
当前边界：自动异地加密备份、备份失败报警、外部 dead-man 和完整生态一键恢复仍需继续建设。后台任务的运行内失败/恢复告警只在本地候选中完成，尚未发布。
...
6. 只读确认 PR #26、`origin/master` 和实时生产；本文记录的 `369024f…` 只是 2026-08-21 验收快照，不能代替当前核验。
...
- [ ] 依赖安装、路由集成测试、完整 API 回归和 Vue 构建等待 GitHub CI
- [ ] 尚未推送、合并、执行迁移 018 或发布生产

### 生产发布

以下勾选项是 `369024f…` 已上线版本的历史验收，不代表当前实时状态：

- [x] 锁定精确 Git merge SHA：`369024f9883a87fb0e1bc05a7665cb2e76437ae2`
...
- OVH 发布目录：`/opt/nav-stack/releases/20260821-165746-369024f`
...
2. 合并并发布后台任务持久状态与运行内失败/恢复告警候选；手工删除不是保留策略的替代品。
...
截至 2026-08-21，账户与审计控制、AI 配置、图床双 Token、主题/无障碍、Session、
缓存/代理、设置/导航、视觉收敛、迁移 017 和两个后台任务已经发布到 OVH SHA
369024f。当前本地分支 codex/nav-maintenance-observability-20260822 只完成迁移 018、
后台任务状态与运行内 Telegram 失败/恢复告警，尚未推送或发布。先只读核对
origin/master、实时生产 SHA、双域、容器健康、备份和当前数据库边界；不要推断 Oracle
旧个人数据已恢复。
