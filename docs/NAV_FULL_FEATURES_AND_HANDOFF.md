...
- 主分支：`master`。
- 2026-08-21 当前本地只读核验的 GitHub `origin/master` 基线为
  `a9fe1ab0b59db8c3e33bcc8456b1a17b6e5a00fa`（PR #25 合并点）。
- 未完成功能本地候选位于 `codex/nav-unfinished-p1-20260821`：安全审计分层保留与导出、
  图片删除失败后台退避重试、日志脱敏与容器日志限额。该候选尚未推送、合并或发布，
  两个后台任务在示例配置中均默认关闭。
- 2026-08-21 GitHub `master`：`42bd430320e1c656170bf90c6d97700ebaede2b4`。
- GitHub PR `#18`、`#19`、`#20`、`#21` 均已合并；PR `#20/#21` 的分支与 PR 事件 CI 均通过。
- PR `#20` 合入主题对比度、弹窗无障碍、Session single-flight/内存复用、统一 401、路由进度与骨架、缓存/真实 IP/canonical、六分类设置页和统一导航。
- PR `#21` 合入紧凑搜索、横向书签卡、统一页面宽度与图片库/时光视觉收敛。
- 当前已核验的生产应用代码仍为 `d0d432b4e0b2e50c25fccb7b45fc609be18c31a0`；PR `#20/#21` **尚未发布生产**。
...
- 图床双 Token 生产配置已在配置发布 `/opt/nav-stack/releases/20260821-094700-d0d432b4-imgbed` 修复并验收；Token 值不进入 Git、文档或浏览器。对应 release-local Secret 挂载源码已由 PR `#20` 合并，但仍需随下一次应用发布进入生产镜像。
...
- 本地候选新增按当前筛选导出 CSV/JSON；单次最多 10,000 条，导出行为也会写入审计
- 本地候选新增 90/180/365 天分层保留任务，使用跨副本锁和有界批次，生产启用前必须
  先做数据库备份、隔离恢复与过期数量复核
...
- 图片清理失败可手动重试；本地候选增加按指数退避的后台重试，且每次重试前重新确认
...
6. 只读确认 PR `#18` 至 `#21`、`origin/master` 和实时生产；GitHub 主分支与生产 SHA 当前不同，禁止把“已合并”误写成“已上线”。
...
- [x] PR `#18/#19/#20/#21` 已合并，`master` CI 通过
- [x] PR `#20/#21` 各自的分支事件与 PR 事件完整 CI 均通过
- [x] PR `#20/#21` 新增差异未发现 Token、API Key 或真实 `.env`；仓库唯一规则命中是既有测试样例

### UI、性能与无障碍源码（已合并，尚未发布）

- [x] 六套主题分别提供亮/暗辅助文字 token，并以背景、卡片和输入区域不低于 `4.5:1` 为门禁
...
- [ ] 对 PR `#20/#21` 做生产前数据库备份、隔离恢复、构建产物与 Compose/Nginx 门禁
- [ ] 取得独立发布授权后发布，并完成双域登录态、真实 IP、缓存头、分享 canonical、图床、AI 和手机触控验收
...
以下勾选项是 `d0d432b4…` 已上线版本的历史验收，不代表 PR `#20/#21` 已发布：

- [x] 锁定精确 Git merge SHA：`d0d432b4e0b2e50c25fccb7b45fc609be18c31a0`
- [x] 发布前 PostgreSQL 压缩备份与配置副本完成
- [x] 在隔离、无网络 PostgreSQL 容器中完成恢复演练和表计数校验
- [x] 数据库迁移数量与发布源码一致；API 启动迁移成功
...
- [x] 模型目录、服务端托管非秘密元数据和一次 Responses API 真实调用通过
- [x] 失败时可按固定旧 SHA `86ede7a001d585ea929e698afdcae0976a3cbde0` 回滚 API/Web；PostgreSQL 与 CLIProxyAPI 不参与应用回滚

### 生产发布证据

- OVH 发布目录：`/opt/nav-stack/releases/20260820-105500-d0d432b4`
- 源码归档 SHA-256：`02e8016bda31d0f31d8bd2c20e10b6f94d1c57955f767c504e3fccf43127a182`
- 生产前 PostgreSQL 备份 SHA-256：`86d2f2784e68cbd59859682ffdc2dbc9b0690aeaa10a4c37648b3d123046fbc0`
- 生产前门禁、切换和验收摘要：发布目录的 `evidence/PRE_SWITCH.txt`、`evidence/SWITCH.txt`、`evidence/ACCEPTANCE.txt`
- 仅应用回滚脚本：发布目录的 `ovh/rollback-nav-app.sh`
...
账户用户名与密码修改、安全审计默认折叠和受保护删除、AI 服务器托管/自定义配置
来源切换已经发布到 OVH，图床双 Token 配置也已修复。PR #20/#21 的主题、无障碍、
Session、缓存/代理、设置/导航和视觉收敛只完成了 GitHub 合并，尚未发布生产。先只读
核对 origin/master、实时生产 SHA、双域、容器健康、备份和当前数据库边界；不要重复
发布，也不要推断 Oracle 旧个人数据已恢复。
