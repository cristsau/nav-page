...
- 本轮账户修改、审计删除和 AI 配置来源切换属于 **本地候选**；在 GitHub CI、合并、备份和生产验收完成前，不得写成“已上线”。
...
- [ ] 删除后存在新的“删除安全审计记录”事件
- [ ] 手机端按钮、密码输入和复选框可触达
- [ ] AI“配置来源”默认显示服务器托管，当前 Base URL 与 Endpoint 可见但只读
- [ ] 切换自定义 API 后启用、格式、地址和密钥输入可操作
- [ ] 自定义 API 未保存密钥时得到清晰错误，不回退借用服务器 Secret
- [ ] 切回服务器托管后模型目录与连接测试继续使用只读 Secret
