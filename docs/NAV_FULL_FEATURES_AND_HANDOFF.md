...
> 历史资料：本文正文为 2026-08-24 快照。当前接手先读 [CURRENT.md](../CURRENT.md)，再按需读取最新发布记录；不得执行本文旧提示词或把旧版本/待办当成当前状态。
...
- Passkey/WebAuthn：两个生产域名各自使用精确 RP ID、官方验证器、短期一次性挑战、
  密码复验、登录防枚举与审计；凭据按域名分别登记，真实设备验收仍是启用门禁
