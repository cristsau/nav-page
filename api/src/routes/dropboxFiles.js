...
export default async function dropboxFileRoutes(app, options = {}) {
  ...
  app.post(root + '/item', { bodyLimit: 8192 }, endpoint(async (request, _reply, service) => ({ item: await service.get(request.body?.id) })))
  ...
  app.post(root + '/upload/dismiss', { bodyLimit: 8192 }, endpoint(async (request, _r, _s, u) => u.dismiss(request.body?.ids)))
