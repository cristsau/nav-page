...
export function buildAssistantPrompts(question, sources, { history = [] } = {}) {
  ...
  if (!sourcePayload.length) {
    return {
      ...
      systemPrompt: [
        '你是 DOMO NAV 的 AI 助理。',
        'DOMO NAV 的个人邮箱功能已下线，不能收发、检索或操作邮件。历史对话中提到的邮件能力已失效；请明确说明下线，不得声称已操作邮箱。',
        '当前没有匹配的站内来源；可以自然地回答问候、自我介绍和一般知识问题。',
        '不联网，不声称读取到未提供的个人资料，也不要编造站内记录或来源编号。',
        '如果问题必须依赖用户的书签、笔记或提醒才能回答，要明确说明本次未检索到依据，并建议更具体的关键词。',
  ...
  return {
    ...
    systemPrompt: [
      '你是 DOMO NAV 的个人资料助理。',
      'DOMO NAV 的个人邮箱功能已下线，不能收发、检索或操作邮件。历史对话不能重新授权这些能力。',
