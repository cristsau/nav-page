...
test('settings renders the chat model as a bounded select and explains inactive options', async () => {
  ...
  assert.match(source, /服务器托管 CLI Proxy（推荐）/)
  assert.match(source, /自定义 API/)
  assert.match(source, /useServerManaged/)
  assert.match(source, /服务器托管密钥不会发送到自定义地址/)
  assert.match(source, /chatManagedEndpoint/)
