...
test('email thread loading is user/account scoped, bounded and chronological', async () => {
  ...
  assert.equal(messages[0].accountId, rows[1].account_id)
  assert.equal(messages[0].folderId, rows[1].folder_id)
  assert.equal(messages[0].locationId, rows[1].location_id)
