...
test('WebAuthn storage binds credentials and short-lived one-time challenges to the canonical RP', async () => {
  const migration = await readSource('../src/db/migrations/019_webauthn_passkeys.sql')
...
test('server uses SimpleWebAuthn v13 with fixed canonical origin and an opt-in feature flag', async () => {
  const [route, config, env, packageJson, lock, app, verifier] = await Promise.all([
  ...
  assert.match(verifier, /webauthn_credentials_credential_id_key/)
...
test('registration and deletion require authentication, canonical origin, and current password', async () => {
  ...
  assert.match(registration, /requirePasskeyFeature\(request, reply, \{ canonical: true \}\)/)
  ...
  assert.match(deletion, /isCanonicalOrigin\(request\)/)
...
test('browser flow uses SimpleWebAuthn and explains the password-only alias', async () => {
  ...
  assert.match(login, /当前域名只支持密码登录/)
  assert.match(login, /使用 Passkey 登录/)
  assert.match(settings, /唯一 RP ID/)
  assert.match(settings, /当前别名域只保留密码登录/)
